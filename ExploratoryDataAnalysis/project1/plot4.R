...
png(filename="~/Desktop/Coursera/ExploratoryDataAnalysis/project1/plot4.png",height=480,width=480,bg="white")
par(mfrow=c(2,2),mar=c(4,4,2,2))
plot4a=with(alldata,plot(datetime,Global_active_power,type="l",ylab="Global Active Power (kilowatts)",xlab=""))
plot4b=with(alldata,plot(datetime,Voltage,type="l"))
plot4c=with(alldata,{plot(datetime,Sub_metering_1,type="n",ylab="Energy sub metering",xlab="") + points(datetime,Sub_metering_1,type="l") + points(datetime,Sub_metering_2,type="l",col="red")+points(datetime,Sub_metering_3,type="l",col="blue") + legend("topright",c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),lty=1,cex=.75,col=c("black","red","blue"),bty="n")})
plot4d=with(alldata,plot(datetime,Global_reactive_power,type="l"))
dev.off()